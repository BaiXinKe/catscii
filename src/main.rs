...
use opentelemetry::{
    ...
    trace::{get_active_span, FutureExt, Status, TraceContextExt, Tracer},
    Context, KeyValue,
};
...
async fn main() {
    ...
    let (_honeyguard, _tracer) = opentelemetry_honeycomb::new_pipeline(
        std::env::var("HONEYCOMB_API_KEY").expect("$HONEYCOMB_API_KEY shouled be set"),
        "catscii".into(),
    )
    .install()
    .unwrap();
...
async fn dowload_file(client: &reqwest::Client, url: &str) -> color_eyre::Result<Vec<u8>> {
    let bytes = client
        .get(url)
        .send()
        .await?
